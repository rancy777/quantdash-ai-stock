...
const GlassCard: React.FC<GlassCardProps> = ({ children, className = '', title, action, noPadding = false }) => {
  return (
    <div className={`relative backdrop-blur-md rounded-2xl overflow-hidden flex flex-col transition-all duration-300 shadow-xl 
      ...
      ${className}`}>
      {(title || action) && (
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-200/50 dark:border-white/5 flex-shrink-0">
          {title && <h3 className="text-lg font-semibold text-slate-800 dark:text-gray-100 tracking-wide flex items-center gap-2">
