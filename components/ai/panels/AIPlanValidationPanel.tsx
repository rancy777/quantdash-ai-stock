...
import { AIPlanValidationEntry } from '../../../services/aiDailyReviewService';
import { renderStructuredDocument, SELECT_CLASS_NAME } from '../config';
import Badge from '../../ui/Badge';
import GlassCard from '../../ui/GlassCard';

type AIPlanValidationPanelProps = {
  canGenerate: boolean;
  generating: boolean;
  error: string;
  history: AIPlanValidationEntry[];
  selectedHistoryId: string;
  validation: AIPlanValidationEntry | null;
  editing: boolean;
  draft: string;
  editError: string;
...
export default function AIPlanValidationPanel({
  canGenerate,
  generating,
  error,
  history,
          ...
          <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/10 p-4">
            <div className="mb-4 rounded-xl border border-cyan-300/40 bg-white/60 p-4 dark:border-cyan-500/20 dark:bg-white/5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <Badge variant="blue">结论：{validation.summary.verdictLabel}</Badge>
                <Badge variant="outline">观察标的 {validation.summary.observedCount} 只</Badge>
                <Badge variant="green">强于预期 {validation.summary.strongCount}</Badge>
                <Badge variant="outline">中性 {validation.summary.neutralCount}</Badge>
                <Badge variant="red">弱于预期 {validation.summary.weakCount}</Badge>
              </div>
              {validation.summary.observedSymbols.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {validation.summary.observedSymbols.map((symbol) => (
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-3">
                <Badge variant="purple">{validation.providerName}</Badge>
                <Badge variant="outline">计划交易日 {validation.targetTradingDate}</Badge>
                <Badge variant="outline">校验交易日 {validation.validationDate}</Badge>
                <span className="text-xs text-slate-500 dark:text-gray-400">
                  生成于 {new Date(validation.generatedAt).toLocaleString()}
                </span>
              </div>
            ...
            {editing ? (
              ...
              renderStructuredDocument(validation.content, 'cyan')
            )}
          </div>
        )}
